...
import {deleteNetworkEdge,deleteNetworkNode,updateNetworkEdge,updateNetworkNode} from "./domain/network-editor.js";
import {auditGuiyunState} from "./domain/product-guardrails.js";

const interfaceStyles=document.createElement("link");interfaceStyles.rel="stylesheet";interfaceStyles.href="/assets/styles/ui.css";document.head.append(interfaceStyles);
...
$("#ai-submit").onclick=async()=>{
  ...
  const nodes=ids.map(id=>n.nodes.find(node=>node.id===id)).filter(Boolean).map(node=>({id:node.id,title:node.title,note:node.note,excerpt:node.anchor?.selectedText||node.source.excerpt})),button=$("#ai-submit");button.disabled=true;button.textContent="知乎直答生成中…";
  try{const data=await request("/api/zhihu/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({task,nodes,existingEdges:n.edges,edgeId})}),batchId=crypto.randomUUID();if(task==="connect"){for(const edge of data.result.edges??[])n.proposals.push({id:crypto.randomUUID(),batchId,kind:"edge",status:"pending",payload:edge,demo:data.demo})}else n.proposals.push({id:crypto.randomUUID(),batchId,kind:task,status:"pending",nodeId:writeNodeId||null,edgeId:edgeId||null,targetNodeIds:ids,payload:data.result,demo:data.demo});state.activeNetworkId=n.id;selected.clear();saveState(state);renderNetworkSelector();renderNetwork();$("#ai-dialog").hidden=true;toast(data.demo?"已生成演示草案":"知乎直答已生成待确认草案")}catch(error){toast(error.message)}finally{button.disabled=false;button.textContent="生成待确认草案"}
};
...
$("#integrate-new").onclick=async()=>{
  ...
  const candidateIds=new Set(candidates.map(node=>node.id)),nodes=integrationCatalog(n,candidateIds).map(node=>({id:node.id,title:node.title,note:node.note,excerpt:node.anchor?.selectedText||node.source.excerpt})),button=$("#integrate-new");button.disabled=true;button.querySelector("span").textContent="正在整合…";
  try{
    const data=await request("/api/zhihu/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({task:"integrate",nodes,candidateNodeIds:[...candidateIds],existingEdges:n.edges})}),batchId=crypto.randomUUID();
    for(const edge of data.result.edges??[])n.proposals.push({id:crypto.randomUUID(),batchId,kind:"edge",status:"pending",payload:edge,demo:data.demo,integrationCandidateNodeIds:[...candidateIds]});
  ...
  }catch(error){toast(`整合失败：${error.message}`)}finally{button.disabled=false;button.querySelector("span").textContent="一键整合新增节点"}
};

function renderNetwork(){
  const n=network(),newCount=newIntegrationNodes(n).length;$("#active-network-name").textContent=n.name;$("#node-count").textContent=n.nodes.length;$("#edge-count").textContent=n.edges.length;$("#new-node-count").textContent=newCount;$("#integrate-new").disabled=newCount===0;$("#integrate-new").title=newCount?`${newCount} 个新增节点等待整合`:"没有待整合的新增节点";$("#summary").textContent=n.summary||"AI 不会创建节点；连接和总结都需要你确认。";
  ...
  if(!n.nodes.length){g.innerHTML='<div class="graph-watermark">归云网络</div><div class="graph-empty"><b>从一次主动归档开始</b><p>阅读、选择、确认，然后知识才进入网络。</p></div>';$("#node-detail").innerHTML="<p>这个网络还是空的。阅读并手动归档内容后，节点会出现在这里。</p>";renderProposals();return}
...
function showNode(n){
  ...
  $("#node-detail").innerHTML=`<div class="detail-heading"><span class="badge">${n.type==="article"?"整篇":"文段"}</span><span class="object-kind">${n.integrationStatus==="new"?"待整合 · ":""}节点</span></div><h3>${esc(n.title)}</h3>${n.anchor?`<blockquote>${esc(n.anchor.selectedText)}</blockquote>`:""}${n.note?`<p>${esc(n.note)}</p>`:""}<small>${esc(n.source.author)} · ${esc(n.source.title)}</small><div><a class="button" id="jump-local" href="${readerUrl}">返回阅读位置</a>${sourceLink}</div><div class="object-actions"><button id="center-object" ${isCenter?"disabled":""}>${isCenter?"当前中心":"设为中心"}</button><button id="edit-object">编辑节点</button><button id="delete-object" class="danger">删除节点</button></div>`;
