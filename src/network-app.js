...
function compactWorkspace(){return matchMedia("(max-width: 1050px)").matches}
function rerenderAfterLayout(){requestAnimationFrame(()=>requestAnimationFrame(renderNetwork))}
function closeWorkspaceDrawers(){workspaceLayout.classList.remove("left-open","right-open");syncWorkspaceLayout()}
function syncWorkspaceLayout(){
  const compact=compactWorkspace();
  if(compact)workspaceLayout.classList.remove("left-collapsed","right-collapsed");
  else{workspaceLayout.classList.toggle("left-collapsed",workspaceLayoutState.leftCollapsed);workspaceLayout.classList.toggle("right-collapsed",workspaceLayoutState.rightCollapsed);workspaceLayout.classList.remove("left-open","right-open")}
  const leftVisible=compact?workspaceLayout.classList.contains("left-open"):!workspaceLayoutState.leftCollapsed,rightVisible=compact?workspaceLayout.classList.contains("right-open"):!workspaceLayoutState.rightCollapsed;
  $(".network-navigation").setAttribute("aria-hidden",String(!leftVisible));$(".network-inspector").setAttribute("aria-hidden",String(!rightVisible));
  $("#expand-left-panel").hidden=compact?false:leftVisible;$("#expand-right-panel").hidden=compact?false:rightVisible;
  $("#network-panel-scrim").hidden=!(compact&&(leftVisible||rightVisible));
}
function saveWorkspaceLayout(){localStorage.setItem(WORKSPACE_LAYOUT_KEY,JSON.stringify(workspaceLayoutState))}
function collapsePanel(side){if(compactWorkspace()){workspaceLayout.classList.remove(`${side}-open`)}else{workspaceLayoutState[`${side}Collapsed`]=true;saveWorkspaceLayout()}syncWorkspaceLayout();rerenderAfterLayout()}
function expandPanel(side){if(compactWorkspace()){workspaceLayout.classList.remove(side==="left"?"right-open":"left-open");workspaceLayout.classList.add(`${side}-open`)}else{workspaceLayoutState[`${side}Collapsed`]=false;saveWorkspaceLayout()}syncWorkspaceLayout();rerenderAfterLayout()}
function setEditMenu(open){const menu=$("#network-edit-menu"),toggle=$("#network-edit-toggle");menu.hidden=!open;toggle.setAttribute("aria-expanded",String(open));if(open)menu.querySelector("button:not(:disabled)")?.focus()}
$("#collapse-left-panel").onclick=()=>collapsePanel("left");$("#collapse-right-panel").onclick=()=>collapsePanel("right");$("#expand-left-panel").onclick=()=>expandPanel("left");$("#expand-right-panel").onclick=()=>expandPanel("right");$("#network-panel-scrim").onclick=closeWorkspaceDrawers;
$("#network-edit-toggle").onclick=event=>{event.stopPropagation();setEditMenu($("#network-edit-menu").hidden)};
$("#network-edit-menu").addEventListener("click",event=>{if(event.target.closest("button"))setEditMenu(false)});
document.addEventListener("click",event=>{if(!event.target.closest(".network-edit-group"))setEditMenu(false)});
syncWorkspaceLayout();
...
document.addEventListener("keydown",event=>{
  ...
  if(currentView?.overviewExpanded){currentView.overviewExpanded=false;renderNetwork();event.preventDefault();return}
...
let aiConfig={providers:[],trial:{available:false,loginRequired:true}},pendingAiAction=null;
function readJsonStorage(storage,key){try{return JSON.parse(storage.getItem(key)||"{}")??{}}catch{return{}}}
function readAiSettings(){const preferences=readJsonStorage(localStorage,AI_PREFERENCES_KEY),secrets=readJsonStorage(sessionStorage,AI_SECRETS_KEY),fallbackProvider=aiConfig.providers.some(item=>item.id==="openai-next")?"openai-next":(aiConfig.providers[0]?.id||"deepseek"),provider=aiConfig.providers.some(item=>item.id===preferences.provider)?preferences.provider:fallbackProvider,definition=aiConfig.providers.find(item=>item.id===provider),model=definition?.models.includes(preferences.models?.[provider])?preferences.models[provider]:definition?.models[0]||"";return{mode:preferences.mode==="own"?"own":"trial",provider,model,apiKey:String(secrets.apiKeys?.[provider]||"")}}
...
function ensureAiConfigured(resume){const settings=readAiSettings(),trialReady=settings.mode==="trial"&&aiConfig.trial.available&&!aiConfig.trial.loginRequired,ownReady=settings.mode==="own"&&Boolean(settings.apiKey.trim());if(trialReady||ownReady)return true;pendingAiAction=resume;openAiSettings();document.querySelector('input[name="ai-mode"][value="own"]').checked=true;syncAiModeFields("own");if(aiConfig.providers.some(item=>item.id==="openai-next")){const current=readAiSettings();$("#ai-provider").value="openai-next";renderAiModels(current.provider==="openai-next"?current.model:"")}$("#ai-api-key").focus();toast("请输入 API Key，保存后将自动继续");return false}
...
$("#ai-settings").onclick=()=>{pendingAiAction=null;openAiSettings()};$("#ai-settings-close").onclick=$("#ai-settings-cancel").onclick=()=>{pendingAiAction=null;$("#ai-settings-dialog").hidden=true};
...
$("#integrate-new").onclick=async()=>{
  ...
  if(!ensureAiConfigured(()=>$("#integrate-new").click()))return;
...
function renderNetwork(){
  ...
  g.querySelectorAll("[data-edge]").forEach(path=>{path.onclick=event=>{event.stopPropagation();if(view.selectionMode)return;const edge=n.edges.find(item=>item.id===path.dataset.edge);if(edge){focused={kind:"edge",id:edge.id};showEdge(edge);renderNetwork()}};path.onpointerenter=()=>path.previousElementSibling?.classList.add("hovered");path.onpointerleave=()=>path.previousElementSibling?.classList.remove("hovered")});
  renderProposals();
}
...
function renderAssociationOverview(root,n,view,w,h){
  ...
  view.overviewPanX??=0;view.overviewPanY??=0;view.overviewScale??=1;
  const edges=n.edges.map((edge,index)=>{const a=pos.get(edge.sourceNodeId),b=pos.get(edge.targetNodeId);return a&&b?`<g class="overview-edge-group ${focused?.kind==="edge"&&focused.id===edge.id?"focused":""}" data-overview-edge="${edge.id}" role="button" tabindex="0" aria-label="关系：${esc(edge.title||relationLabel(edge.relationType))}" style="--overview-delay:${Math.min(index*28,360)}ms"><line class="overview-edge" x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"/><line class="overview-edge-hit" x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}"/></g>`:""}).join("");
  const nodes=n.nodes.map((node,index)=>{const p=pos.get(node.id),classes=[node.id===view.centerId?"center":"",selected.has(node.id)?"selected":"",focused?.kind==="node"&&focused.id===node.id?"focused":""].filter(Boolean).join(" ");return`<g class="overview-node ${classes}" data-overview-node="${node.id}" role="button" tabindex="0" aria-label="节点：${esc(node.title)}" style="--overview-delay:${Math.min(index*34,420)}ms"><circle class="overview-node-hit" cx="${p.x}" cy="${p.y}" r="${expanded?15:8}"/><circle class="overview-node-dot" cx="${p.x}" cy="${p.y}" r="${node.id===view.centerId?5:3.5}"/><circle class="overview-node-ring" cx="${p.x}" cy="${p.y}" r="${node.id===view.centerId?9:7}"/><title>${esc(node.title)}</title></g>`}).join("");
  const nodeLabels=expanded?n.nodes.map(node=>{const label=labels.get(node.id);return label?`<text class="overview-node-label ${focused?.kind==="node"&&focused.id===node.id?"focused":""}" data-overview-label="${node.id}" x="${label.x}" y="${label.y}">${esc(label.title)}</text>`:""}).join(""):"";
  root.insertAdjacentHTML("beforeend",`<section class="association-overview ${expanded?"expanded":""}" aria-label="${expanded?"全局知识图谱画布":"全局知识图谱缩略图"}" data-overview-expanded="${expanded}"><button class="overview-toggle" aria-label="${expanded?"还原全局知识图谱缩略图":"最大化全局知识图谱"}" title="${expanded?"还原缩略图":"最大化全局网络"}">${expanded?"↙":"↗"}</button>${expanded?`<div class="overview-controls" aria-label="全局网络缩放"><button data-overview-zoom="in" aria-label="放大全局网络">＋</button><button data-overview-zoom="out" aria-label="缩小全局网络">－</button><button data-overview-zoom="reset" aria-label="重置全局网络位置">◎</button></div>`:""}<svg viewBox="0 0 ${ow} ${oh}" aria-label="全部节点缩略图"><g class="overview-viewport" transform="translate(${view.overviewPanX} ${view.overviewPanY}) scale(${view.overviewScale})">${edges}${nodes}${nodeLabels}</g></svg></section>`);
  const overview=root.querySelector(".association-overview"),svg=overview.querySelector("svg"),viewport=overview.querySelector(".overview-viewport");
  overview.querySelector(".overview-toggle").onclick=()=>{view.overviewExpanded=!view.overviewExpanded;renderNetwork()};
  const activateNode=(element)=>{const node=n.nodes.find(item=>item.id===element.dataset.overviewNode);if(!node)return;if(view.selectionMode){selected.has(node.id)?selected.delete(node.id):selected.add(node.id)}else{focused={kind:"node",id:node.id};centerOnNode(view,node.id);showNode(node)}renderNetwork()};
  const activateEdge=(element)=>{if(view.selectionMode)return;const edge=n.edges.find(item=>item.id===element.dataset.overviewEdge);if(!edge)return;focused={kind:"edge",id:edge.id};showEdge(edge);renderNetwork()};
  overview.querySelectorAll("[data-overview-node]").forEach(element=>{const label=overview.querySelector(`[data-overview-label="${element.dataset.overviewNode}"]`);element.onclick=event=>{event.stopPropagation();activateNode(element)};element.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();activateNode(element)}};element.onpointerenter=()=>label?.classList.add("hovered");element.onpointerleave=()=>label?.classList.remove("hovered")});
  overview.querySelectorAll("[data-overview-edge]").forEach(element=>{element.onclick=event=>{event.stopPropagation();activateEdge(element)};element.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();activateEdge(element)}}});
  overview.querySelectorAll("[data-overview-zoom]").forEach(button=>button.onclick=()=>{const action=button.dataset.overviewZoom;if(action==="reset"){view.overviewScale=1;view.overviewPanX=0;view.overviewPanY=0}else{const next=Math.max(.5,Math.min(4,view.overviewScale+(action==="in"?.25:-.25))),ratio=next/view.overviewScale;view.overviewPanX=ow/2-(ow/2-view.overviewPanX)*ratio;view.overviewPanY=oh/2-(oh/2-view.overviewPanY)*ratio;view.overviewScale=next}renderNetwork()});
  installOverviewInteraction(overview,svg,viewport,view,ow,oh);
}
function installOverviewInteraction(overview,svg,viewport,view,ow,oh){
  let drag=null;
  const renderTransform=()=>viewport.setAttribute("transform",`translate(${view.overviewPanX} ${view.overviewPanY}) scale(${view.overviewScale})`);
  svg.onpointerdown=event=>{if(event.button!==0||event.target.closest("[data-overview-node],[data-overview-edge],button"))return;event.preventDefault();const rect=svg.getBoundingClientRect();drag={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,panX:view.overviewPanX,panY:view.overviewPanY,ratioX:ow/rect.width,ratioY:oh/rect.height};svg.setPointerCapture?.(event.pointerId);overview.classList.add("dragging")};
  svg.onpointermove=event=>{if(!drag||drag.pointerId!==event.pointerId)return;view.overviewPanX=drag.panX+(event.clientX-drag.startX)*drag.ratioX;view.overviewPanY=drag.panY+(event.clientY-drag.startY)*drag.ratioY;renderTransform()};
  const finish=event=>{if(!drag||drag.pointerId!==event.pointerId)return;svg.releasePointerCapture?.(event.pointerId);drag=null;overview.classList.remove("dragging")};svg.onpointerup=finish;svg.onpointercancel=finish;
  svg.addEventListener("wheel",event=>{event.preventDefault();const rect=svg.getBoundingClientRect(),cursorX=(event.clientX-rect.left)*ow/rect.width,cursorY=(event.clientY-rect.top)*oh/rect.height,delta=Math.max(-80,Math.min(80,event.deltaY)),next=Math.max(.5,Math.min(4,view.overviewScale*Math.exp(-delta*.0035))),ratio=next/view.overviewScale;view.overviewPanX=cursorX-(cursorX-view.overviewPanX)*ratio;view.overviewPanY=cursorY-(cursorY-view.overviewPanY)*ratio;view.overviewScale=next;renderTransform()},{passive:false});
}
function showNode(n){
...
init();addEventListener("resize",()=>{syncWorkspaceLayout();renderNetwork()});
